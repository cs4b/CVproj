...
def AOS_integrator(LFR_dir, source_dir, download_dir, focal_plane):
    """
    Integrates the LFR images using the AOS Renderer
    :param LFR_dir: Directory containing the LFR set
    :param source_dir: Directory containing the images to be integrated
    :param download_dir: Directory to save the integrated images
    :param focal_planes: plane to integrate image on (integers) not greater than 0 which is the ground level
    :return: None
    """

    ## path to where the results will be stored 

    Download_Location = download_dir    ## Enter path to the directory where you want to save the results.
    print(Download_Location)
    Integral_Path = os.path.join(Download_Location, os.path.dirname(source_dir)) # Note that your results will be saved to this integrals folder.
    ...
    set_folder = LFR_dir         # Enter path to your LFR/python directory
    ...
    for img in sorted(glob.glob(source_dir + '/*.png'),key=numericalSort):      # Enter path to the images directory which should contain 11 images.
        n= cv2.imread(img)
        imagelist.append(n)
...
sep_dir = sorted(os.listdir(r'complete_sets_separated'))
focal_planes = [0, -0.8, -1.6, -2.4]
for dir in tqdm(sep_dir, desc='integrating dir {dir} at various focal planes'):
#   print(f'start folder {sep_dir[i]}')
    if '.DS_Store' not in os.path.basename(dir) and 'GT' not in os.path.basename(dir):
        for plane in focal_planes:
            AOS_integrator(r'../LFR', r'complete_sets_separated/'+sep_dir[dir], r'integrated/'+sep_dir[dir], plane)
            print(f' integrated focal plane {plane} for dir {dir}')
    print(f'integrated dir :{dir}')